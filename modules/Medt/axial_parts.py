...
class AxialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.width:
            x = x.permute(0, 2, 1, 3) # N0, H2, C1, W3
        else:
            x = x.permute(0, 3, 1, 2)  # N0, W3, C1, H2
        ...
        #torch.split(input, split_size_or_sections, dim ):将输入张量（input）沿着指定维度（dim）分割成特定数量的张量块，并返回元素为张量块的元素。
        
        # Calculate position embedding
        all_embeddings = torch.index_select(self.relative, 1, self.flatten_index).view(self.group_planes * 2, self.kernel_size, self.kernel_size)
        q_embedding, k_embedding, v_embedding = torch.split(all_embeddings, [self.group_planes // 2, self.group_planes // 2, self.group_planes], dim=0)
        #print("q_embedding.shape:{};k_embedding.shape:{};v_embedding.shape:{}".format(q_embedding.shape,k_embedding.shape,v_embedding.shape))
        #torch.index_select(input,dim,index,out=None) 函数返回的是沿着输入张量(input)的指定维度(dim)的指定索引号(index)进行索引的张量子集(out)

        qr = torch.einsum('bgci,cij->bgij', q, q_embedding)
        kr = torch.einsum('bgci,cij->bgij', k, k_embedding).transpose(2, 3)
        qk = torch.einsum('bgci, bgcj->bgij', q, k)
        #print("qr.shape:{};kr.shape:{};qk.shape:{}".format(qr.shape,kr.shape,qk.shape))
        ...
        #print("AxialAttentionoutput.shape:{}".format(output.shape))
...
class AxialBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.hight_block = AxialAttention(width, width, groups=groups, kernel_size=kernel_size)#纵向，height
        self.width_block = AxialAttention(width, width, groups=groups, kernel_size=kernel_size, stride=stride, width=True)#横向，width
    ...
    def forward(self, x):
        ...
        out += identity
        out = self.relu(out)
        #print("AxialBlockoutput.shape:{}".format(out.shape))
